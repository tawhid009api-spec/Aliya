	...
	/*
		...
		*/
		...
		async function onStart() {
			...
			// —————————————— CHECK PREFIX & ADMIN LOGIC —————————————— //
			const isBotAdmin = role >= 3; // Role 3 = AdminBot, Role 4 = Developer
			let hasPrefix = body.startsWith(prefix);

			// Normal users der jonno prefix thaka obligatory
			if (!hasPrefix && !isBotAdmin)
				return;

			let workingBody = body;
			if (hasPrefix) {
				workingBody = body.slice(prefix.length).trim();
			} else {
				workingBody = body.trim();
			}

			const dateNow = Date.now();
			const args = workingBody.split(/ +/);
			...
			// Admin ra prefix chara random kicho likhle "Command Not Found" error avoid korar jonno
			if (!hasPrefix && isBotAdmin && !command) {
				return;
			}
			...
			function removeCommandNameFromBody(body_, prefix_, commandName_) {
				if (arguments.length) {
					...
					const currentPrefix = hasPrefix ? prefix : "";
